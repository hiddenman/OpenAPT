...
class Publishing(Entity):
    ...
    def run(self):
        ...
        if not self.context.execute(['publish', 'show', self.distribution] + optional_args, 1, False):
            ...
            if not self.context.execute(switch_args + ['publish', 'switch'] + args):
                raise AptlyException()
            return
        ...
        if self.origin:
            extra_args.append('-origin=%s' % self.origin)

        if self.label:
            extra_args.append('-label=%s' % self.label)

        if self.suite:
            extra_args.append('-suite=%s' % self.suite)
